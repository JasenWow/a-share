"""Vendored Kronos model neural network modules from shiyu-coder/Kronos.

Source: https://github.com/shiyu-coder/Kronos/blob/master/model/module.py
License: MIT

Only modified: removed sys.path manipulation, changed to local imports.
"""
...
class BinarySphericalQuantizer(nn.Module):
    def __init__(self, embed_dim, beta, gamma0, gamma, zeta,
                 ...
                 cb_entropy_compute='group', l2_norm=True,
                 inv_temperature=1):
        super().__init__()
...
class DependencyAwareLayer(nn.Module):
    ...
    def forward(self, hidden_states, sibling_embed, key_padding_mask=None):
        attn_out = self.cross_attn(
            query=sibling_embed, key=hidden_states, value=hidden_states,
...
class DualHead(nn.Module):
    ...
    def compute_loss(self, s1_logits, s2_logits, s1_targets, s2_targets, padding_mask=None):
        if padding_mask is not None:
            ...
            s1_logits, s2_logits = s1_logits[valid_mask], s2_logits[valid_mask]
            s1_targets, s2_targets = s1_targets[valid_mask], s2_targets[valid_mask]
