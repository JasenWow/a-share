"""Vendored Kronos model classes from shiyu-coder/Kronos.

Source: https://github.com/shiyu-coder/Kronos/blob/master/model/kronos.py
License: MIT

Only modified: fixed import path to use local module.py instead of sys.path hack.
"""
...
from big_a.models.kronos_model.module import (
    BSQuantizer,
...
def _sample_from_logits(logits, temperature=1.0, top_k=None, top_p=None, sample_logits=True):
...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context,
    ...
    with torch.no_grad():
        ...
        ran = trange(pred_len) if verbose else range(pred_len)
        for i in ran:
...
class KronosPredictor:
    ...
    def predict_batch(self, df_list, x_timestamp_list, y_timestamp_list, pred_len,
        ...
        if not isinstance(df_list, (list, tuple)):
            raise ValueError("df_list must be list or tuple.")
        ...
        for i in range(num_series):
            ...
            x_time_df = calc_time_stamps(x_timestamp_list[i])
            y_time_df = calc_time_stamps(y_timestamp_list[i])
            ...
            x_stamp_list.append(x_time_df.values.astype(np.float32))
            y_stamp_list.append(y_time_df.values.astype(np.float32))
            means.append(x_mean)
            stds.append(x_std)
            seq_lens.append(x_norm.shape[0])
            y_lens.append(y_time_df.shape[0])

        if len(set(seq_lens)) != 1:
            raise ValueError(f"Inconsistent historical lengths: {seq_lens}")
        if len(set(y_lens)) != 1:
            raise ValueError(f"Inconsistent prediction lengths: {y_lens}")
